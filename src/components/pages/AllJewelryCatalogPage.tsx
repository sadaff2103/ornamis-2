...
export function AllJewelryCatalogPage({ onNavigate, onBack }: AllJewelryCatalogPageProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    let result = getTimeSeededShuffle(allProducts);
    ...
    >
            ...
            <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
              <SheetTrigger asChild>
                ...
                >
                  ...
                  Filters
                  {hasActiveFilters && (
                    <span className="ml-1 bg-[#d4af37] text-[#1E1E1E] text-[10px] rounded-full size-4 flex items-center justify-center font-bold">!</span>
                  )}
                </Button>
              </SheetTrigger>
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          <Sparkles className="size-8 text-[#d4af37]" />
        ...
        <p className="text-white/80 text-base md:text-lg max-w-3xl">
          Explore our complete catalog — rings, necklaces, earrings and bracelets from GIVA, Palmonas and Khan Jewellers.
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        className="max-w-7xl mx-auto px-4 mb-6 space-y-4"
      >
        ...
        <div className="max-w-2xl relative">
          ...
          <input
            ...
            placeholder="Search by name, store, material…"
          ...
          />
          {localQuery && (
            <button onClick={() => setLocalQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 hover:text-white">
              <X className="size-4" />
            </button>
          )}
        </div>
        ...
        <div className="flex gap-6 lg:gap-8">
          ...
          >
            <div className="sticky top-4">
              <FilterSidebar onFilterChange={setFilters} onNavigate={onNavigate} />
            </div>
          </motion.aside>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-white">
            <div>
              ...
              <div className="space-y-2 text-sm">
                {(["rings", "necklaces", "earrings", "bracelets"] as const).map(cat => (
                  <button key={cat} onClick={() => onNavigate(cat)} className="block capitalize hover:underline text-white/80 hover:text-white transition-colors">
                    {cat}
                  </button>
                ))}
                <button onClick={() => onNavigate("ai-designer")} className="block hover:underline text-white/80 hover:text-white transition-colors">AI Designer</button>
              </div>
            </div>
            <div>
            ...
            <div className="flex flex-col items-start md:items-end">
              ...
              <button onClick={() => onNavigate("privacy")} className="text-sm text-white/60 hover:text-white transition-colors">
